...
struct APIClient {
    ...
    static func getStuff(for urlString: String,
                         completion: @escaping (Result<Horoscope, AppError>) -> ()) { //it takes in a valid url and a completion handler
        ...
        NetworkHelper.shared.performDataTask(request: urlRequest) { (result) in
            ...
            case .success(let data):
                do {//any function that uses an error you must try//anytime you decode data from JSON you need to include .self
                    let searchResults = try JSONDecoder().decode(Horoscope.self, from: data)
